...
class JudgeServerHeartbeatAPI(CSRFExemptAPIView):
    ...
    def post(self, request):
        # print("JudgeServerHeartbeatAPI", request.data)
        data = request.data
        ...
        # print("✅ SERVER INFO SAVED/UPDATED SUCCESSFULLY")
        try:
...
#     @validate_serializer(JudgeServerHeartbeatSerializer)
#     def post(self, request):
#         import hashlib  # 상단에 import 추가 필요
#         import os  # 필요하다면 추가
        
#         # 환경 변수 확인
#         print("=== ENV DEBUG ===")
#         env_token = os.environ.get("JUDGE_SERVER_TOKEN", "없음")
#         print(f"환경 변수 JUDGE_SERVER_TOKEN: {env_token}")
#         print("=== END ENV DEBUG ===")
...
#         # 토큰 디버깅 코드 시작
#         client_token = request.META.get("HTTP_X_JUDGE_SERVER_TOKEN")
#         server_token = SysOptions.judge_server_token
#         hash_token = hashlib.sha256(server_token.encode("utf-8")).hexdigest()
        
#         print(f"=== TOKEN DEBUG ===")
#         print(f"Client token: {client_token}")
#         print(f"Server token: {server_token}")
#         print(f"Hashed server token: {hash_token}")
#         print(f"Tokens match: {client_token == hash_token}")
